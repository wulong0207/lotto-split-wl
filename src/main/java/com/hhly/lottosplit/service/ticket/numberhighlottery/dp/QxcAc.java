 ...
 * @Description: 七星彩
 * @author wuLong
 * @date 2017年7月31日 下午2:39:27 
 *
 */
@Component
public class QxcAc extends AbstractCSplitHanlde implements AbstractINumberSplit {
	@Autowired
			...
			if(size<4){
				...
				if(subNum>1){
					...
					if(bcs==3){
						...
						y[1] = StringUtils.join(c.subList(subNum, bcs*2),SymbolConstants.COMMA);
						y[2] = StringUtils.join(c.subList(bcs*2, c.size()),SymbolConstants.COMMA);
					}else{
						y[0] = StringUtils.join(c.subList(0, subNum),SymbolConstants.COMMA);
